...
public class Excursao {
	...
	public Excursao(int codigo) throws Exception {				// Construtor com 1 parâmetro
		carregar();
	...
	public Excursao(int codigo, double preco, int maxReservas) throws Exception { 		// Construtor com 3 parâmetros
		if (codigo <= 0) 																// Lança uma exceção se 'codigo' <= 0
			throw new Exception("O código da excursão deve ser maior do que 0");
	...
	public void cancelarReserva(String cpf) throws Exception {
		...
		// ArrayList auxiliar para onde as reservas NÃO RELACIONADAS com o cpf serão
		// armazenadas
		...
		for (String reservaAtual : reservas) {
			...
			if (partes.length == 2 && partes[0].equals(cpf)) {
				...
				// Caso a reserva em questão não esteja associada ao CPF, mantém a reserva na
				// lista de reservas não removidas
	...
	public ArrayList<String> listarReservasPorCpf(String cpf) throws Exception {
		...
		// Caso contrário, iremos procurar se alguma reserva bate com 'cpf'
			...
			// ArrayList auxiliar para armazenar as reservas que batem com o 'cpf' passado
			// no método
			ArrayList<String> reservasPorCpf = new ArrayList<>();
			for (String reservaAtual : reservas) {
				String[] partes = reservaAtual.split("/");
				if (partes.length == 2 && partes[0].equals(cpf)) {
					// Caso alguma reserva possua o 'cpf' igual, esta reserva será adicionada ao
					// ArrayList auxiliar
...
//		// O objeto 'arq' é criado para escrever no arquivo representado por f. O
//		// argumento 'true' indica que novos dados serão adicionados ao final do
//		// arquivo, preservando os dados existentes
//		FileWriter arq = new FileWriter(f, true);
//		// Contém as informações solicitadas pelo professor que deveriam ser gravadas no
//		// arquivo .csv
...
//	}
	...
	public void carregar() throws Exception {
		try {
			...
			while(arquivo.hasNextLine()) {
				...
				partes = linha.split(";");
				if (!partes[2].equals("disponivel")) {
					String[] partesReserva = partes[2].split("/");
					criarReserva(partesReserva[0], partesReserva[1]);
				}
			}
			...
			File f = new File(new File(".\\" + getCodigo() + ".csv").getCanonicalPath());
			FileWriter arquivo2 = new FileWriter(f, false);
			arquivo2.close();
		}
	}
	...
	public void gravar() throws Exception {
		try {
			// Tenta abrir o arquivo "codigo.csv", localizado na pasta do projeto, usando um
			// objeto 'File'
			File f = new File(new File(".\\" + getCodigo() + ".csv").getCanonicalPath());
			// Em seguida, utiliza um 'FileWriter' para escrever no arquivo. O parâmetro
			// 'false' indica que o arquivo deve ser aberto para escrita, substituindo o
			// conteúdo anterior
			FileWriter arquivo = new FileWriter(f, false);
			// Grava a primeira linha do arquivo (linha de cabecalho contendo o Preço e o Nº
			// máximo de reservas da excursão)
			arquivo.write("Preço; Max; Reserva\n");
			// Variável responsável por armazenar a reserva
			String reservaAtual;
			// Percorre o ArrayList 'reservas', verificando se uma reserva está ocupada (ou
			// seja, não é "disponivel"). Se uma reserva estiver ocupada, escreve o número
			// da reserva e a o registro correspondente no arquivo .csv
